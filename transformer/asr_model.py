...
#    calculate aed_prob_dist, r_aed_prob_dist ,then calculate loss_att
    def cal_aed_prob_dist(
        self,
        speech: torch.Tensor,
        ...
        encoder_out, encoder_mask = self.encoder(
            speech, speech_lengths, decoding_chunk_size=-1, num_decoding_left_chunks=-1)
        # Attention Decoder
        if self.ctc_weight != 1.0:
            ...
            aed_prob_dist = None
            r_aed_prob_dist = None

            if self.reverse_weight < 1.0:
                decoder_out = decoder_out / temperature
                aed_prob_dist = decoder_out.log_softmax(dim=2)
            elif self.reverse_weight > 0.0:
                r_decoder_out = r_decoder_out / temperature
                r_aed_prob_dist = r_decoder_out.log_softmax(dim=2)
        else:
            aed_prob_dist = None
            r_aed_prob_dist = None

        return aed_prob_dist, r_aed_prob_dist


#    calculate ctc_prob_dist ,then calculate loss_ctc, "frame"
    def cal_ctc_prob_dist(
        self,
        speech: torch.Tensor,
        speech_lengths: torch.Tensor,
        temperature: float = 1.0,
        ...
        if self.ctc_weight != 0.0:
            encoder_out = encoder_out / temperature
            ctc_prob_dist = self.ctc.log_softmax(encoder_out)
        else:
            ctc_prob_dist = None
        return ctc_prob_dist

#    calculate ctc_prob_dist ,then calculate loss_ctc, "secquence_sample"
    def create_ctc_seq_sampler(
        self,
        speech: torch.Tensor,
        speech_lengths: torch.Tensor,
        temperature: float = 1.0,
        ...
        if self.ctc_weight != 0:
            ...
            ctc_seq_sampler = Categorical(logits=ctc_logits)
        else:
            ctc_seq_sampler = None
        return ctc_seq_sampler
...
#    calculate ctc_prob_dist ,then calculate loss_ctc, "secquence_sample"
    def sample_ctc_seq(
        ...
        if sampler is not None:
            ...
            for i in range(sample_num):
                sample_index = sampler.sample() # [B, maxlen]
                for hyp in sample_index:
                    hyps.append(hyp.tolist())
            hyps = [torch.Tensor(remove_duplicates_and_blank(hyp)).int() for hyp in hyps]
            text = pad_sequence(hyps, batch_first=True, padding_value=IGNORE_ID)
            text_lengths = torch.Tensor([len(hyp) for hyp in hyps]).int()
        else:
            text = None
            text_lengths = None
        return text, text_lengths # [B * sample_num, maxlen], [B * sample_num]
        
    
    def ctc_greedy_search_for_KD(
        self,
        speech: torch.Tensor,
    ...
    if self.ctc_weight != 0.0:
        assert speech.shape[0] == speech_length.shape[0]
        encoder_out, encoder_mask = self.encoder(
            speech, speech_length, decoding_chunk_size = -1, num_decoding_left_chunks = -1)
        maxlen = encoder_out.size(1)
        ctc_logits = self.ctc_lo(encoder_out)
        ctc_logits = ctc_logits / temperature
        ctc_probs = ctc_logits.log_softmax(dim=2)
        ...
        text = None
        text_lengths = None
    return text, text_lengths # [B, maxlen], [B,]
